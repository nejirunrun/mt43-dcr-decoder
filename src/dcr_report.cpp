...
//   ここは「32byte → 値」だけを担当し、受信（GPSControl）・保管（notice_store）・
//   表示（display）を一切知らない。依存は Arduino.h と自ヘッダのみ。
//
//   元は GPSControl.cpp の dumpDcrLoop() 内に埋まっていた診断コード。CLAUDE.md
//   「テストプログラムの実装合格後、src/下に個別フォルダを作成し API 昇格」に従って
//   切り出した（2026-07-28）。切り出しにあたり、実測で判明した指紋の不具合
//   （PAB 混入で同一内容が別 CRC になる）を修正している＝旧診断コードの
//   CRC 指紋方式は本ファイルの fingerprint に置き換わった。
//
// 【非自明な定数の根拠】
...
//     valid/mt/fingerprint までしか埋めない（jmaValid=false）。
//   ・震央地名（10bit）の名称表は dcr_epicenter_table.cpp に全件収録済み（2026-07-28）。
//   ・詳細デコーダは EEW(1)・震源(2)・気象(10) のみ。他カテゴリは共通部だけ得られる。
//     ★座標を積むのは震源(2)だけ。EEW(1) は地域コードで「どこが揺れるか」を伝える形式で、
//       構造的に座標を持たない（bit43〜209 が隙間なく埋まっており入る余地がない）。
// =============================================================================
...
//   ★なぜ生バイト/CRC ではないのか（碑文・2026-07-28）:
//     MT43 には確定した区切り・長さ・順番が無く、観測側からその構造を知る術がない。
//     同じ通報がカルーセルしているように「見える」だけで、ビット列が同一である保証は
//     どこにも無い。実測でも、同一内容（熊本の余震・同じ発生時刻/震央/M/座標）の再送が
//     3回とも別のビット列だった。よって「どのビットをマスクすれば安定するか」を探す
//     アプローチは原理的に解に到達しない。デコードして意味が確定したものだけを鍵にする。
//
...
uint32_t dcr_content_key(const DcrReport& r) {
    ...
    // カテゴリ別詳細。★2026-07-30 全14カテゴリ分を追加（旧実装は EEW/震源/気象の
    //   3カテゴリしか無く、残り11カテゴリは共通部（報告時刻まで）だけでキーが決まって
    //   いた。台風で実際に事故が起きた: 実況/予報レグが同じ報告時刻を共有し、
    //   位置・気圧・風速だけが違う複数メッセージが同一キーに丸め込まれ、SD保全の
    //   重複判定（dcr_raw_log.cpp・1起動中の初出キーだけ保存）で予報レグが静かに
    //   欠落しかけた。生ログ解析で発覚（頻繁な瞬断リブートが起動セッションを短く
    //   区切っていたため、この夜は実害が出る前に境界を跨いで事なきを得ていた）。
    if (r.category == DCR_CAT_EEW) {
        ...
        DcrTyphoon ty;
        if (dcr_typhoon(r, ty)) {
            // ★このカテゴリで実際に事故が起きた: 実況/予報レグが同じ報告時刻を
            //   共有し、位置・気圧・風速だけが違う複数メッセージが同一キーに
            //   丸め込まれ、SD保全の重複判定（1起動中の初出キーだけ保存）で
            //   予報レグが静かに欠落した（2026-07-30 生ログ解析で発覚）。
...
//
//   ビット配置（IS-QZSS-DCR §4.1.2.3.1 ／ azarashi 実装と照合・2026-07-28）。
...
//     転記後に必ず確認すること。FIT フッタのフィールド番号転記事故と同型の地雷。
// ─────────────────────────────────────────────────────────────
...
//
//   ビット配置（azarashi qzss_dcr_decoder_jma_hypocenter.py と照合・2026-07-28）。
...
//
//   ビット配置（azarashi qzss_dcr_decoder_jma_seismic_intensity.py と照合・2026-07-29）:
...
//
//   ビット配置（azarashi qzss_dcr_decoder_jma_tsunami.py と照合・2026-07-29）:
...
//   IS-QZSS-DCR §4.1.2.3.10 ＋ azarashi 実装と一致確認済み（2026-07-27）。
//     +53  3bit  警戒状態（発表/解除）
...
//   ビット配置（azarashi qzss_dcr_decoder_jma_nankai_trough_earthquake.py と照合・2026-07-30）:
...
//   ビット配置（azarashi qzss_dcr_decoder_jma_northwest_pacific_tsunami.py と照合・2026-07-30）:
...
//   ビット配置（azarashi qzss_dcr_decoder_jma_volcano.py と照合・2026-07-30）:
...
//   ビット配置（azarashi qzss_dcr_decoder_jma_ash_fall.py と照合・2026-07-30）:
//     +53  5+5+6  活動時刻 日/時/分                  → 53..68
...
//   ビット配置（azarashi qzss_dcr_decoder_jma_flood.py と照合・2026-07-30）:
...
//   ビット配置（azarashi qzss_dcr_decoder_jma_typhoon.py と照合・2026-07-30）:
...
//   ビット配置（azarashi qzss_dcr_decoder_jma_marine.py と照合・2026-07-30）:
...
// コード → 名称テーブル
// ─────────────────────────────────────────────────────────────
// ── 通報区分（Rc・3bit）★2026-07-30 全面修正 ───────────────────────────────
//   【一次資料】IS-QZSS-DCR-016 Table 4.1.2-5 他（各カテゴリの Parameter Definitions）
//     Rc: 1=Maximum priority / 2=Priority / 3=Regular / 7=Training/Test
//
//   ★旧実装は 0=最大優先/1=優先/2=通常/3=訓練 と**1つずれており、case 7 も欠けていた**。
//     結果、実災害の「通常(3)」を「訓練/試験」と誤表示し、本物の訓練(7)は "?" になっていた。
//     台風13号（実在）が [試験] と出たことでユーザーが気づき発覚（2026-07-30）。
//     これは表示だけの問題ではない: dcr_board の isDrill が classification==3 を見ており、
//     **実災害情報を「訓練だから通知しない」と握りつぶす**安全上の欠陥になっていた。
//
//   【4,5,6 と 0 が未割当な理由】（一次資料からの読み取り）
//     Table 4.1.1-1 の送信レートでは 訓練/試験 は「優先」と同じ 4倍（通常の1に対し4）で
//     送られる＝**低優先度ではない**。つまり 1-2-3 は優先度の序列で、7 はその序列の外に
//     ある「実在しない放送」のマーカー（全ビット1のセンチネル＝本仕様書全体の慣用）。
//     4,5,6 は序列を拡張する余地として空けてあると読める。
//     ★よって 7 を「通常より下の優先度」として序列に組み込んではいけない（別軸）。
//
...
//   SD ログの補助・EPD 表示・シリアル出力で同じ文面を使うためにここへ置く。
...
// 実際に有効な値かを示す（一次資料 Table 4.1.2-30 Parameter Definitions (Volcano)）。
//   ★azarashi はこの値を生の番号のまま保持するだけで日本語訳を持たない
//     （名称表という体裁の表ではなく Parameter Definitions 内の説明列であるため）。
//     一次資料の英文説明から直接訳出した。
...
// ★level==1「警報解除」が明示的な終了値。掲示板モデルの状態バケツ分類の根拠。
...
// ★code==0「海上警報解除」が明示的な終了値。掲示板モデルの状態バケツ分類の根拠。
...
// 地方海上予報区（14bit・49件）。azarashi qzss_dcr_jma_marine_forecast_region.py と
//   照合済み（2026-07-30）。表が小さいので他の大きい表と違いここへ直置きする。
