...
//
// 【出典 — 一次資料から機械転記】
//   IS-QZSS-DCR-016 Table 4.1.2-6 "Notifications on Disaster Prevention"（全 55 件）。
//   ★このファイルは **tools/dcr_spec/dcr_tables.json から機械転記**したものであり、
//     人が目で写していない。JSON 自体も一次資料 PDF から機械抽出している
//     （人手が入るのは配置だけ）。
//   ★整合は照合ツールが守る。表を編集したら必ず流すこと:
//         python tools/dcr_spec/dcr_spec.py verify
//     手で書き換えて一次資料からズレると、通報区分(Rc)の1つズレ事故と同型の
//     「静かな誤情報」になる。EEW はその他全ての通知に優先する情報であり、
//     ここでの誤認は許されない。
//
// 【なぜ全件そろえて入れるか】
...
//   結合してある（折り返しは組版の都合＝EPD の幅で改めて折り返せばよい。
//   README「データは発信者の内容を尊重し、活用は利用者側で再構築する」に従う）。
